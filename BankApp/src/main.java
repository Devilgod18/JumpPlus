...
public class main {
    ...
    public static void main(String[] args) {
        System.out.println("Welcome to the Simplified Banking App!");
        UserAccountDAO userAccountDAO;
        try {
            userAccountDAO = new UserAccountDAO();
        } catch (SQLException | ClassNotFoundException e) {
            System.err.println("Error initializing UserAccountDAO:");
            e.printStackTrace();
            return;
        }
        ...
        while (true) {
            ...
            switch (choice) {
                case 1:
                    register(userAccountDAO);
                    break;
                case 2:
                    login(userAccountDAO);
                    break;
                case 3:
    ...
    private static void register(UserAccountDAO userAccountDAO) {
    ...
    private static void login(UserAccountDAO userAccountDAO) {
    ...
    private static void deposit(UserAccount userAccount, UserAccountDAO userAccountDAO) throws SQLException {
        ...
        if (userAccountDAO.deposit(userAccount.getUsername(), amount)) {
            ...
            System.out.println("Deposit successful! Your new balance is: " + userAccountDAO.getBalance(userAccount.getUsername()));
        } else {
    ...
    private static void withdraw(UserAccount userAccount, UserAccountDAO userAccountDAO) throws SQLException {
        ...
        if (userAccountDAO.withdraw(userAccount.getUsername(), amount)) {
            ...
            System.out.println("Withdrawal successful! Your new balance is: " + userAccountDAO.getBalance(userAccount.getUsername()));
        } else {
    ...
    private static void transfer(UserAccount userAccount, UserAccountDAO userAccountDAO) throws SQLException {
        ...
        if (userAccountDAO.transfer(userAccount.getUsername(), recipientUsername, amount)) {
            ...
            System.out.println("Transfer successful! Your new balance is: " + userAccountDAO.getBalance(userAccount.getUsername()));
        } else {
    ...
    private static void viewRecentTransactions(UserAccount userAccount, UserAccountDAO userAccountDAO) throws SQLException {
        ...
        if (transactions.isEmpty()) {
            ...
            for (Transaction transaction : transactions) {
                System.out.printf("%s: %s %s (ID: %d, Amount: %.2f, Date: %s)\n",
                    transaction.getType(),
